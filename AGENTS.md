...
- 新增错误码通过新建 `@RegisterException` 装饰的叶节点异常类扩展，不修改现有错误处理逻辑
- 错误通过 `AppException` 子类（配合 `@RegisterException` 装饰器注册）抛出，不返回模糊的通用错误

### LSP — 里氏替换原则

子类型必须能替换其基类而不破坏程序行为：

- 自定义异常继承 `AppException`（或其子类 `ClientException` / `InfraException` / `SystemException`）时，保持错误响应格式一致
...
**异常注册表** — `src/common/exceptions/` — 抛出异常前先查各域 `*.exception.ts` 是否已有对应 `@RegisterException` 叶节点类，运行时通过 `ErrorRegistry` 查询全量注册数据
...
`code` 字段对应各域 `*.exception.ts` 中 `@RegisterException` 装饰器所注册的 `code`，运行时由 `ErrorRegistry` 统一管理。
...
- [ ] 已确认错误码是否已在各域 `src/**/*.exception.ts` 中通过 `@RegisterException` 注册
